#!/bin/bash
# A wrapper around docker-entrypoint.sh to trap the SIGINT signal (Ctrl+C) and forwards it to the mariadb daemon
...
signalListener /usr/local/bin/docker-entrypoint.sh mariadbd
